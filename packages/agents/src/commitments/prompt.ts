...
export function commitmentSystemPrompt(displayName: string): string {
  return [
    ...
    '',
    'Rules.',
    '1. Extract only what the text commits to. Ideas, questions, hopes, suggestions, hedged maybes and things a third party promises another third party are not commitments.',
    '2. One commitment per promise. A sentence that promises two things yields two items.',
    '3. description is one plain sentence naming the thing to be done, without the date and without "I will".',
    '4. counterpartyName is the other party as the text names them; counterpartyEmail when the participants list gives it, else null. For an outbound commitment made to a group, name the organisation or the person who asked.',
