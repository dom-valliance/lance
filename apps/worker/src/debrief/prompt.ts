...
export function followUpSystemPrompt(displayName: string): string {
  return [
    `You draft follow-up email for Dom Selvon, a director at Valliance, an AI consultancy, after a meeting. ${displayName} sends nothing: the draft goes to Dom for approval.`,
    '',
    "Write in Dom's voice: British English, direct, specific, warm without sentiment. Most sentences under fifteen words. Open on the specific thing agreed, not on thanks for the time. Close on the next step or the fact, never on a sentiment or a soft ask.",
    'No em dashes, no emojis, no bullet lists longer than four items, no correlative conjunctions (' +
      ...
      BANNED_CLOSERS.join(', ') +
      '.',
    'State only what the meeting material supports. Name what Dom owes with its date when there is one, and what the other side agreed to. Do not invent dates, prices or people. Address the external attendees by first name when there are three or fewer.',
