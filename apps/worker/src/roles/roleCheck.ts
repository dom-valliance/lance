 ...
 * every active principal who holds neither, with a ledger event and a P1
 * alert for each admin.
 *
 * Graph application permissions it needs, both admin-consented by
...
export interface RoleCheckResult {
  /** Entra object ids holding each role, directly or through a group. */
  holders: Record<LanceRole, string[]>;
  paused: { principalId: string; upn: string }[];
  /** Active principals with no Entra object id yet, so nothing to check them against. */
  unbound: string[];
...
/** Who holds each Lance role: direct user assignments and every user in an assigned group. */
...
export async function runRoleCheck(options: RoleCheckOptions): Promise<RoleCheckResult> {
  ...
  if (anyRole.size === 0) {
    throw new RoleCheckError(
      'Graph reports nobody holding a Lance app role, so the role check paused nobody. Check the group assignments on the Lance enterprise application (docs/runbooks/entra-setup.md section 8).',
    );
  }

  const active = await options.root
  ...
  for (const principal of active) {
    ...
    const updated = await admin
      ...
      .where(and(eq(principals.id, principal.id), eq(principals.status, 'active')))
    ...
    for (const recipient of adminPrincipals) {
      await raiseAlert(scopedDb(options.root, { principalId: recipient.id }), {
        ...
        body: `The nightly role check found that ${principal.upn} holds neither Lance.User nor Lance.Admin in Entra, so Lance paused them. Their watchers and jobs stop. If the removal was a mistake, add them back to the Lance Users group and set their status to active.`,
        actor: ROLE_CHECK_ACTOR,
        correlationId,
        now: () => ts,
      });
