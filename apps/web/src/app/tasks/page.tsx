...
export default async function TasksPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const [page, proposals] = await Promise.all([
    ...
    client.proposals.list.query({ status: 'pending', actionClass: 'complete_task', limit: 200 }),
  ]);
  const tasks = page.items;
  const pending = pendingCompletionFor(tasks, proposals);
                    ...
                    <Tr key={task.id} muted={task.done}>
                      <Td>
                        ...
                        <p className="mt-1 text-xs text-muted-foreground">
                          {sourceStatusLine(task)}
                        </p>
                      </Td>
                      <Td>
                        <Badge tone={SYSTEM_TONES[task.source]}>
                          {sourceBadgeLabel(task.source)}
                        </Badge>
                      </Td>
                      <Td>
                        {task.due === null ? (
