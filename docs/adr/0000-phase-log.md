...
| `/lance status` answers from production | Dev environment deployed 2026-09-21 (rg-lance-dev, images 9137265): six migrations and the seed applied by the migrate identity, api `/health/ready` 200 as its managed identity, web sign-in redirect correct, worker started in dry_run with pg-boss's tables created under its own identity. Slack `/lance status`, `pause` and `resume` from Dom pending; those write the first production `state_changed` events. | 2026-09-21, Slack step pending |
