...
| Migration run against dev before merge | Rehearsed by the phase review on a throwaway container loaded with dev-shaped rows, as a NOSUPERUSER `lance_migrator` member; `migrate.backfill.test.ts` covers a users row whose id is not the seed's. A run against a copy of dev's own data is pending Dom's go-ahead. | |
