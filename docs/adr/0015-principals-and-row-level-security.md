...
**Columns.** `principal_id char(26) NOT NULL REFERENCES principals(id) DEFAULT nullif(current_setting('app.principal', true), '')` on `ledger_events`, `observations`, `proposals`, `policy_decisions`, `cursors`, `commitments`, `briefs`, `alerts`, `agent_runs` and `principal_state`. Existing rows are backfilled with Dom's id. `policy_rules.principal_id` is nullable, and null means an organisation default. The default replaces the briefing's `DROP DEFAULT`: a row takes its principal from the session scope, never from a caller argument, and an unscoped insert fails the NOT NULL. Uniques that were global become per principal: `ledger_events (principal_id, idempotency_key)`, `observations (principal_id, idempotency_key)`, `cursors (principal_id, watcher, key)`, `alerts (principal_id, dedupe_key)`.

**Policies.** Every principal-bearing table has `ENABLE` and `FORCE ROW LEVEL SECURITY` and one policy, `USING (principal_id = current_setting('app.principal', true)) WITH CHECK (the same)`. On `policy_rules`, `USING` also admits `principal_id IS NULL`, and `WITH CHECK` admits null only when `current_setting('app.role', true) = 'admin'`. `lance_app` has no `BYPASSRLS`. The ledger immutability trigger adds `principal_id` to the columns retention may not change. pg-boss tables are untouched.
