...
Each principal connects their own Microsoft 365 (ADR 0022). The connect route is behind Entra bearer authentication, so it is started from the web app's Settings page: sign in, open Settings and press Connect Microsoft 365. The button goes to the web app's own `/api/graph/connect`, which calls the api with the session's token; the api stores a one-use consent state in Postgres (ten minutes, under the principal's scope, so either api replica can finish it) and sends the browser to the api's `/auth/graph/begin`, which sets an HttpOnly, SameSite=Lax cookie on the api's hostname and forwards the browser to Microsoft with the principal's UPN as the login hint. Typing the api URL into a browser does not work. An active principal, or one still onboarding (multi-user M3 step 2), may connect; a paused or offboarded one is refused.

The callback needs that cookie, so a consent link forwarded to someone else fails. The api exchanges the code, checks that the id token Microsoft returns names the principal's own Entra object (`oid` equal to their bound `entra_oid`) in this tenant, and refuses otherwise, storing nothing and recording a `graph_consent_refused` event: a colleague who consents on someone else's link is turned away. Then it stores the refresh token in the principal vault as `graph-refresh-token--<principalId>`, for the principal who started the consent and nobody else. It holds only the custom role `Lance principal secret writer` there, so it can write the secret and never read it back. It records a `graph_token_storing` event before the write and a `state_changed` ledger event with `change: graph_connected` after it. The worker reads the secret, and every refresh rotates it under a Postgres advisory lock for that principal. `/lance status` then shows the Graph connector as connected.
